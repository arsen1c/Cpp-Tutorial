/*
                                            PS: READ RELATED ARTICLES ON GFG;
...
void allocArray()
{
    ...
    int n = 5, i, j, x;
    int *point = NULL;
    point = new(nothrow) int[n];

    if(!point) cout << "Memory allocation failed\n";
    else
    {
        // Increment the value at each iteration by 1
        for(i = 0; i < n; i++) point[i] = i + 1;

        cout << "Value stored in block of mem: ";
        for(j = 0; j < n; j++) cout << " " << point[j];
    }
...
void oneMore()
{
    ...
    ptr = new(nothrow) int[x];

    if(ptr == NULL) cout << "Error: memory could not be allocated\n";
    else
    {
        for(i=0; i < x; i++)
        {
